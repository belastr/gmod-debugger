...
local function logs()
    ...
    logFile.DoClick = function(s)
        ...
        local suc = file.Write(os.date("gmod-debugger/logs/error/l_%Y-%m-%d_%H-%M-%S.txt", os.time()), content)
        if suc then
...
language.Add("error.post", "post logs to the discord (requires webhook configuration)")
