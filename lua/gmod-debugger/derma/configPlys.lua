...
function PANEL:Init()
    ...
    self.btn.DoClick = function()
        ...
        popup.lblTitle:SetFont("Default")
        ...
        btnAdd:Dock(TOP)
        btnAdd:SetFont("Default")
        btnAdd:SetText("Add Player")
        btnAdd.DoClick = function()
            local m = DermaMenu()

            for _, ply in ipairs(player.GetHumans()) do
                if self.value[ply:SteamID()] then continue end
                m:AddOption(ply:SteamID() .. " (" .. ply:Nick() .. ")", function() popup.list:AddLine(ply:SteamID()) self.value[ply:SteamID()] = true end)
            end

            m:AddOption("Enter manually", function() Derma_StringRequest(
                "Add Player", "Input the player's SteamID", "",
                function(text) popup.list:AddLine(text) self.value[text] = true end,
                nil, "Add")
            end)
...
vgui.Register("DebuggerConfigPlys", PANEL, "Panel")
